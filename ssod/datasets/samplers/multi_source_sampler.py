...
class GroupMultiSourceSampler(MultiSourceSampler):
    ...
    def __init__(self,
        ...
        self.group_source2iters = [
            {
                source: self._infinite_indices_of_group(source, group)
    ...
    def _infinite_indices_of_group(self, source: int, group: int) -> Iterator[int]:
        indices = self.group2inds_per_source[source][group]
        g = torch.Generator()
        g.manual_seed(self.seed + source * 10 + group)  # 避免相同顺序

        while True:
            if self.shuffle:
                yield from torch.randperm(len(indices), generator=g).tolist()
            else:
                yield from indices
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        while True:
            ...
            for source, num in enumerate(self.num_per_source):
                batch_buffer_per_source = []
                group_iter = self.group_source2iters[group][source]
                group_inds = self.group2inds_per_source[source][group]

                for _ in range(num):
                    idx_in_group = next(group_iter)
                    real_idx = group_inds[idx_in_group] + self.cumulative_sizes[source]
                    batch_buffer_per_source.append(real_idx)
